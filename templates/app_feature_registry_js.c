/*
 * app_feature_registry_js.c - bundled per-runtime feature registry for the
 * compiler-free `hull build` (docs/compiler_free_build.md).
 *
 * INVARIANT per runtime: this is the exact C that
 * feature_compose.gen_app_registry_c("js") generates. It fills the base's
 * weak hl_stdlib_feature_entries() / hl_runtime_feature_factories() seams
 * with strong overrides pointing at the composed libhull_feature-js.a. It
...
extern const HlEntry hl_stdlib_js_entries[];
static const HlEntry *const HL_STDLIB_FEATS[] = { hl_stdlib_js_entries };
...
static const HlRuntimeFactory *const HL_RT_FEATS[] = { &hl_js_factory };
