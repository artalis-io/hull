...
1. **Every external capability is declared.** Language intrinsics (Lua: `string/table/math/utf8/coroutine`; JS: `Object/Array/Math/JSON`) and Hull's intrinsic core are always available; everything else must appear in `manifest.modules`. The intrinsic core is the minimum needed to bootstrap an app: **`hull/app`** alone (the registration API: `app.manifest`, `app.get/post/use`, `app.router`, `app.ws/sse`, `app.every/daily`, `app.main`). `app` stays intrinsic because the manifest itself is expressed via `app.manifest(...)` — it must exist before the manifest is parsed. `hull/log` and `hull/json` were intrinsic in early development and are now declared modules: apps that call `log.X` or `json.X` directly must put `"hull/log@1"` / `"hull/json@1"` in `manifest.modules`. The response helpers (`res:json(...)`) and the runtime's internal JSON marshalling don't require either declaration — they bypass user-visible imports at the C layer.
