...
**Memory64:** The design is: modules compiled with 64-bit memory (`(memory i64 N)`) are detected automatically; Memory64 **requires AOT** (the fast interpreter does not support it); `hull build` passes `--enable-memory64` to wamrc when it detects one; and the `hull_process` ABI becomes `(i64, i64, i64, i64) -> i32` with the runtime dispatching by the module's `is_memory64` flag. **Caveat (tracked in [#318](https://github.com/artalis-io/hull/issues/318)):** `WASM_ENABLE_MEMORY64` is defined only for the vendored WAMR objects, not for Hull's own TUs, so in the shipped build `cap/wasm.c`'s detection and 8-cell marshalling are `#if`-compiled out — `mod->is_memory64` is always 0 and a Memory64 module is currently mis-dispatched with the wasm32 4-cell ABI. The WAMR engine and `wamrc` support Memory64; Hull's cap-layer dispatch to reach it is not yet compiled in. Re-enabling it needs `-DWASM_ENABLE_MEMORY64=1` on the Hull TUs plus the WAMR internal include paths, and its own end-to-end validation pass.
