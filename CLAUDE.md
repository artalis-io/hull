...
| `e2e_htmx_playwright.sh` | Browser-side E2E for `examples/htmx_widgets_register` (every §1.5.g widget) + `examples/hypermedia_photos` via headless Chromium driven by Playwright. Catches things curl can't: CSS actually applies (regression for the Pico-zinc typo bug), htmx swaps fire, widget JS runs under `csp = "htmx"`, confirm dialog opens only for `hx-confirm` elements (not every request). Skips cleanly when node/npm absent; first run downloads ~150 MB to `tests/.playwright/` (gitignored, cached in CI). 12 + 6 assertions. Make target: `make e2e-htmx-playwright`. |
