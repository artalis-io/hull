/**
 * @file cap/db_transport.h
 * @brief Shared SQL-wire byte transport over Keel v3 public primitives.
 *
 ...
 * the synchronous SQL wire clients (cap/pg_conn.c, cap/mysql_conn.c) drive.
 *
 * Extracted from the byte-identical cap/pg_transport.c and cap/mysql_transport.c
 * (see docs/db_transport_extraction.md).
 *
 * Ownership split: the wire client owns the protocol (framing, auth, TLS
 * negotiation, typed decode, migrations, and the stable error tokens). This
 * transport owns only the bytes: name resolution, the connection race, blocking
 * reads/writes, an optional attached TLS session, and close.
 *
 * Heap-allocated + opaque: the transport is a HEAP allocation reached through an
 * opaque handle. The wire client holds an HlDbTransport* in its connection
 * struct, never an embedded value. This is what lets the exceptional
 * non-detachment path (a connect op that will not confirm detachment) leak the
 * WHOLE allocation intentionally and safely - a live op keeps referencing its own
 * storage, so the storage must never be freed under it, and only a
 * separately-owned heap block can be dropped whole. Mirrors the SMTP transport.
 *
 * Scheduling model: KlConnectOp is asynchronous, so the transport owns a PRIVATE,
 * operation-local KlEventCtx used ONLY during connect establishment. The
 * transport pumps that loop to a terminal state, receives the winning descriptor,
 * and then set_blocking()s it. After that point there is no event loop: startup,
 * auth, the TLS handshake, and every query are plain blocking recv / send on the
 * winning fd. Unlike the SMTP transport this uses no KlStream and no post-connect
 * event loop.
 *
 * TLS: TLS stays the shared hl_tls_client_* helpers layered over the
 ...
 * runs no handshake (the wire client owns the SSLRequest negotiation and hands the
 * resulting session here via hl_db_transport_attach_tls).
 *
 * Diagnostic label: connect / adopt take a stable backend label ("pg" / "mysql")
 * as their FIRST argument. It is used ONLY in the rare connect-op non-detachment
 * log_error diagnostics to identify the backend; it never affects behavior and is
 * never a public error token.
 *
 * Ownership: the transport retains a BORROWED immutable KlSocketProvider reference
 * (the default POSIX provider, or a test-supplied one) whose lifetime exceeds the
 * connection, and it OWNS the private KlEventCtx, the embedded KlConnectOp and its
 * timers, the resolved address set (at most KL_CONNECT_MAX_ADDRS, valid through
 * confirmed detachment), the winning descriptor, and the optional attached
 * HlTlsClient. Every descriptor - winner, loser, or failure-path - is disposed
 * through the provider, never a bare close(2).
 *
 * SPDX-License-Identifier: AGPL-3.0-or-later
 */

#ifndef HL_CAP_DB_TRANSPORT_H
#define HL_CAP_DB_TRANSPORT_H
 ...
 * db_transport.c so the exceptional non-detachment path can drop the whole heap
 * block (see the banner). The direct-include unit test sees the full definition
 * for white-box checks. */
typedef struct HlDbTransport HlDbTransport;
 ...
 * (never in the wire client). At most KL_CONNECT_MAX_ADDRS ordered addresses are
 * raced.
 *
 * @p timeout_ms > 0 bounds TCP establishment only: one absolute connect deadline
 * is armed after resolution, and the Happy-Eyeballs stagger does not refresh it.
 * @p timeout_ms <= 0 is TRULY UNBOUNDED (no total deadline, no hidden ceiling):
 * the loop pumps in bounded event-loop increments until the op completes,
 * preserving the wire client's prior select(..., NULL) contract.
 *
 ...
 * the wire client's set_err style; the allocation is either freed (op detached)
 ...
 * API. The provider is validated (required ops + KL_SOCK_CAP_NATIVE_FD) as in
 * connect.
 *
...
/**
 * Attach a TLS session to route subsequent send/recv through. ONE-SHOT and
 * fallible: requires a live descriptor and a non-NULL @p tls, and rejects a
 * second attachment (so an owned session is never silently dropped). @p tls is
 * created and handshaked by the caller over the transport's blocking descriptor;
 * on success ownership transfers to the transport, which frees it in
 * hl_db_transport_close. Returns 0 on success, -1 on rejection (no descriptor,
 ...
 * byte use hl_db_transport_send_all. Returns the byte count (> 0), 0 on orderly
 * close, or -1 on error.
 */
 ...
 * marked closed): the caller may retry hl_db_transport_close later, or drop its
