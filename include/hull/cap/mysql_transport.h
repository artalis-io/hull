/**
 * @file cap/mysql_transport.h
 * @brief MySQL/MariaDB byte transport over Keel v3 public primitives.
 *
...
#ifndef HL_CAP_MY_TRANSPORT_H
#define HL_CAP_MY_TRANSPORT_H
...
/* Opaque, heap-allocated. mysql_conn.c stores a MyTransport* and reaches every field
 * through the API below. The concrete struct is defined in mysql_transport.c so the
...
typedef struct MyTransport MyTransport;
 ...
 * (never in mysql_conn.c). At most KL_CONNECT_MAX_ADDRS ordered addresses are raced.
 *
 ...
 * completes, preserving mysql_connect's prior select(..., NULL) contract.
 *
 ...
 * mysql_conn.c's set_err style; the allocation is either freed (op detached) or, if
 ...
 * connection. This backs the existing hl_my_conn_start(conn, fd, dsn) test API.
 ...
 * lets hl_my_conn_start keep its "takes ownership of fd, closed on every failure"
 * contract with a valid (default) provider.
 */
MyTransport *hl_my_transport_adopt(int fd,
                                   const struct KlSocketProvider *sp_or_NULL,
                                   char *errbuf, size_t errlen);
 ...
 * created and handshaked by the caller (mysql_conn.c) over the transport's blocking
 * descriptor; on success ownership transfers to the transport, which frees it in
 * hl_my_transport_close. Returns 0 on success, -1 on rejection (no descriptor,
 ...
 * byte use hl_my_transport_send_all. Returns the byte count (> 0), 0 on orderly
 * close, or -1 on error.
 */
...
/**
 * Send every byte of @p buf (all-or-error), looping over hl_my_transport_send.
 ...
 * marked closed): the caller may retry hl_my_transport_close later, or drop its
