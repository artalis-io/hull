 ...
 *
 * Generated by `hull keygen release` for the v0.1.0 release. The
 * matching secret half lives in two places only: the maintainer's
 * `~/.hull/keys/release.key` (with offline backup) and the
 * `HULL_RELEASE_KEY` GitHub Actions secret on artalis-io/hull. Never
 * checked into the repo, never logged.
 *
 * hl_release_pubkey_configured() returns 1 for any non-zero key, so
 * `hull update` enforces signature verification from v0.1.0 onward.
 * The pre-v0.1.0 bypass (warn-and-skip on all-zeros) no longer fires.
 *
 * Rotation: changing this value invalidates every prior `hull.sha256.sig`
 * for `hull update` on already-installed binaries. Plan rotations
 * carefully and coordinate with a release that ships the new pubkey
