...
Hull is at **v0.14.0** (2026-08-29). Pre-stable: APIs and the manifest schema may change. Pin versions and read the [changelog](https://github.com/artalis-io/hull/releases) before upgrading. The v0.14.0 release is a Windows stabilization pass: produced Cosmopolitan APE web apps now serve on Windows (and every host), manifest extraction gains project-rooted local-module resolution with hardened security, `hull update` self-replaces safely on a locked Windows executable, and the tool-resolution surfaces (`doctor` / `tools list` / `agent tools`) agree. **Windows upgrade note:** a v0.13.0 Windows binary cannot self-update to v0.14.0 because that fix ships *in* v0.14.0; download `hull-cosmo` from the v0.14.0 release and replace `hull` once manually while it is not running, after which `hull update` works normally. The published v0.14.0 artifact was acceptance-tested on a non-admin, Developer-Mode-off Windows environment ([`docs/release_acceptance_v0.14.0.md`](docs/release_acceptance_v0.14.0.md)). Recent milestones: v0.13.0 (stdlib maturation), v0.10.0 (self-contained `hull build` on stock Windows), v0.9.0 (the fully-composable SLIM base).
