...
#
# Phase 1 (this file, first cut): stand up the test-key hull + platform lib and
# prove the EXISTING platform-sig path works end to end under it (an app builds,
# the gethull cross-check passes, and it boots with the gethull verify ACTIVE -
# no --no-verify-platform). Phases 2-3 (composed block present; tamper is fatal)
# are added as the compose-recorder + runtime verify land.
#
...
# Boot with the gethull verify ACTIVE (no --no-verify-platform). app.main returns
# 3, so a clean boot exits 3; a gethull refusal exits non-3 with a [sig] error.
set +e
"$APP/bin" >/tmp/composed_sig_run.$$ 2>&1
RC=$?
set -e
if grep -qiE "platform.sig|gethull" /tmp/composed_sig_run.$$ 2>/dev/null \
   && grep -qiE "invalid|mismatch|refus|verification failed" /tmp/composed_sig_run.$$ 2>/dev/null; then
    cat /tmp/composed_sig_run.$$; rm -f /tmp/composed_sig_run.$$
    fail "test hull refused a validly-gethull-signed app at boot"
fi
[ "$RC" = 3 ] || { cat /tmp/composed_sig_run.$$; rm -f /tmp/composed_sig_run.$$; fail "app.main should exit 3 (got $RC) under the active gethull verify"; }
rm -f /tmp/composed_sig_run.$$
pass "validly-signed app boots + exits 3 under the ACTIVE gethull verify (test key)"

echo "PASS: composed-sig e2e (Phase 1 - test-key harness + existing platform-sig)"
