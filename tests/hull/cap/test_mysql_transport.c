 ...
 * capability .c; the Makefile rule for this binary EXCLUDES cap_mysql_transport.o
 * from the common link (mirrors test_smtp_transport / test_mysql_conn). This test
 ...
 * test. */
#include "../../../src/hull/cap/mysql_transport.c"
...
static void fk_reset(void)
{
    ...
    g_fk_send_max = 0;
    g_fk_send_n = 0;
    /* The checkpoint counter is a process-global static in mysql_transport.c; reset it
     ...
     * test direct-includes the capability .c under -DHL_MY_TEST_HOOKS. */
    my_test_checkpoint_seq = 0;
}
...
static int fk_resolve(MyTransport *t, const char *host, int port)
{
    (void)host;
    g_fk_resolve_called++;
...
{
    my_test_resolve         = NULL;
    my_test_socket_provider = NULL;
    my_test_checkpoint      = NULL;
    my_test_force_no_detach = 0;
    my_test_force_alloc_fail = 0;
}
...
/* ── pending -> succeed ─────────────────────────────────────────────── */
UTEST(mysql_transport_connect, pending_then_succeed)
{
    fk_reset(); tp_arm_watchdog();
    g_fk_naddr = 1; g_fk_disp[0] = FK_SUCCEED;
    g_fk_resolve_called = 0;
    my_test_resolve = fk_resolve; my_test_socket_provider = &FK_PROVIDER;

    char err[128] = {0};
    MyTransport *t = hl_my_transport_connect("db.example", "5432", 0, &FK_PROVIDER, err, sizeof err);
    ASSERT_TRUE(t != NULL);
    ASSERT_GE(g_fk_n, 1);
    ASSERT_EQ(g_fk_succeeded_idx, 0);
    ASSERT_GE(g_fk_blocking_set, 1);              /* the winner was set_blocking()'d */
    ASSERT_TRUE(hl_my_transport_fd(t) >= 0);
    ASSERT_TRUE(kl_connect_op_is_detached(&t->connect_op));
    ASSERT_EQ(t->ev_ready, 0);                    /* connect-only ev ctx freed       */

    ASSERT_EQ(hl_my_transport_close(t), 0);
    seam_clear(); tp_disarm_watchdog();
}

/* ── immediate connect() success (rc == 0), not EINPROGRESS ─────────── */
UTEST(mysql_transport_connect, immediate_success_rc0)
{
    fk_reset(); tp_arm_watchdog();
    g_fk_naddr = 1; g_fk_disp[0] = FK_IMMEDIATE;
    my_test_resolve = fk_resolve; my_test_socket_provider = &FK_PROVIDER;

    MyTransport *t = hl_my_transport_connect("h", "5432", 0, &FK_PROVIDER, NULL, 0);
    ASSERT_TRUE(t != NULL);
...
UTEST(mysql_transport_connect, stagger_second_address_wins)
{
    fk_reset(); tp_arm_watchdog();
...
UTEST(mysql_transport_connect, all_addresses_fail_detach)
{
    fk_reset(); tp_arm_watchdog();
    ...
    char err[128] = {0};
    MyTransport *t = hl_my_transport_connect("h", "5432", 0, &FK_PROVIDER, err, sizeof err);
    ASSERT_TRUE(t == NULL);
    ASSERT_TRUE(err[0] != '\0');
...
UTEST(mysql_transport_connect, deadline_fires_detaches)
{
    fk_reset(); tp_arm_watchdog();
    ...
    MyTransport *t = hl_my_transport_connect("h", "5432", 150, &FK_PROVIDER, err, sizeof err);
    ASSERT_TRUE(t == NULL);
    ASSERT_TRUE(err[0] != '\0');
...
static void tp_complete_pending_at_tick(MyTransport *t, unsigned idx)
{
...
UTEST(mysql_transport_connect, unbounded_no_hidden_ceiling)
{
    fk_reset(); tp_arm_watchdog();
    g_fk_naddr = 1; g_fk_disp[0] = FK_PENDING;
    my_test_resolve = fk_resolve; my_test_socket_provider = &FK_PROVIDER;
    my_test_checkpoint = tp_complete_pending_at_tick;

    MyTransport *t = hl_my_transport_connect("h", "5432", 0 /* unbounded */,
                                             &FK_PROVIDER, NULL, 0);
    ASSERT_TRUE(t != NULL);
    ASSERT_EQ(g_fk_succeeded_idx, 0);
    ASSERT_EQ(hl_my_transport_close(t), 0);
...
UTEST(mysql_transport_io, io_status_overrides_errno)
{
    fk_reset(); tp_arm_watchdog();
    g_fk_naddr = 1; g_fk_disp[0] = FK_PENDING;
    g_fk_bogus_errno = 1;                 /* connect sets errno = EPERM */
    g_fk_io_status_val = KL_IO_PENDING;   /* but io_status says PENDING */
    my_test_resolve = fk_resolve; my_test_socket_provider = &FK_PROVIDER_IOS;
    my_test_checkpoint = tp_complete_pending_at_tick;

    MyTransport *t = hl_my_transport_connect("h", "5432", 0, &FK_PROVIDER_IOS, NULL, 0);
...
UTEST(mysql_transport_io, send_eintr_retry_via_io_status)
{
    fk_reset(); tp_arm_watchdog();
    int sv[2]; ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
    my_test_socket_provider = &FK_PROVIDER_IOS;
    char err[64] = {0};
    MyTransport *t = hl_my_transport_adopt(sv[0], &FK_PROVIDER_IOS, err, sizeof err);
    ASSERT_TRUE(t != NULL);
    ...
    ssize_t n = hl_my_transport_send(t, msg, sizeof msg);
...
UTEST(mysql_transport_provider, missing_required_op_fails)
{
    fk_reset();
    ...
    char err[128] = {0};
    MyTransport *t = hl_my_transport_connect("h", "5432", 0, &prov, err, sizeof err);
    ASSERT_TRUE(t == NULL);
    ASSERT_TRUE(strstr(err, "provider") != NULL);
}

/* ── provider validation: missing KL_SOCK_CAP_NATIVE_FD fails closed ── */
UTEST(mysql_transport_provider, missing_native_fd_cap_fails)
{
    fk_reset();
    static KlSocketProvider prov;
    ...
    /* adopt validates too. */
    int sv[2]; ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
    MyTransport *ta = hl_my_transport_adopt(sv[0], &prov, err, sizeof err);
    ASSERT_TRUE(ta == NULL);
    close(sv[0]); close(sv[1]);
}

/* ── set_blocking failure fails the connect closed ──────────────────── */
UTEST(mysql_transport_connect, set_blocking_failure_fails)
{
    fk_reset(); tp_arm_watchdog();
    g_fk_naddr = 1; g_fk_disp[0] = FK_SUCCEED;
...
UTEST(mysql_transport_resolve, ip_literal_no_dns)
{
    fk_reset(); tp_arm_watchdog();
    g_fk_disp[0] = FK_SUCCEED;
    g_fk_resolve_called = 0;
    my_test_resolve = NULL;               /* force the real resolve_addrs */
    my_test_socket_provider = &FK_PROVIDER;

    MyTransport *t = hl_my_transport_connect("127.0.0.1", "5432", 0, &FK_PROVIDER, NULL, 0);
    ASSERT_TRUE(t != NULL);
...
UTEST(mysql_transport_io, adopt_roundtrip_close_once)
{
    fk_reset(); tp_arm_watchdog();
    int sv[2]; ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
    my_test_socket_provider = &FK_PROVIDER;
    MyTransport *t = hl_my_transport_adopt(sv[0], &FK_PROVIDER, NULL, 0);
    ASSERT_TRUE(t != NULL);
    ASSERT_EQ(hl_my_transport_fd(t), sv[0]);

    const uint8_t out[4] = { 9, 8, 7, 6 };
    ASSERT_EQ(hl_my_transport_send_all(t, out, sizeof out), 0);
    uint8_t got[4] = {0};
    ssize_t r = recv(sv[1], got, sizeof got, 0);
    ASSERT_EQ((int)r, 4);
    ...
    uint8_t in[2] = {0};
    ASSERT_EQ((int)hl_my_transport_recv(t, in, sizeof in), 2);
 ...
 *    hl_my_conn_start relies on adopt taking ownership of the fd and closing it on
 *    every failure. Force transport_new() to fail (my_test_force_alloc_fail): adopt
 *    must return NULL AND close the descriptor through the provider exactly once,
 *    so the caller never leaks it. ─────────────────────────────────────────────── */
UTEST(mysql_transport_adopt, alloc_failure_consumes_fd_once)
{
    fk_reset();
    int sv[2]; ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
    g_fk_close_n = 0;
    my_test_force_alloc_fail = 1;         /* transport_new() returns NULL */

    char err[64] = {0};
    MyTransport *t = hl_my_transport_adopt(sv[0], &FK_PROVIDER, err, sizeof err);
...
UTEST(mysql_transport_io, partial_recv_short_count)
{
    fk_reset(); tp_arm_watchdog();
    int sv[2]; ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
    my_test_socket_provider = &FK_PROVIDER;
    ...
    uint8_t buf[16] = {0};
    ssize_t n = hl_my_transport_recv(t, buf, sizeof buf);  /* asked 16, gets 2 */
    ASSERT_EQ((int)n, 2);
 ...
 *    send() calls: this proves hl_my_transport_send_all loops over short writes
...
UTEST(mysql_transport_io, send_all_completes_forced_partial_writes)
{
    fk_reset(); tp_arm_watchdog();
    int sv[2]; ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
    my_test_socket_provider = &FK_PROVIDER;
    ...
    g_fk_send_n   = 0;
    ASSERT_EQ(hl_my_transport_send_all(t, payload, sizeof payload), 0);   /* all sent */
...
UTEST(mysql_transport_tls, attach_one_shot_and_fallible)
{
    fk_reset(); tp_arm_watchdog();
    int sv[2]; ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, sv), 0);
    my_test_socket_provider = &FK_PROVIDER;
    ...
    ASSERT_EQ(hl_my_transport_attach_tls(t, NULL), -1);
    ...
    ASSERT_EQ(hl_my_transport_attach_tls(t, sentinel), 0);
    ...
    ASSERT_EQ(hl_my_transport_attach_tls(t, other), -1);
...
UTEST(mysql_transport_tls, attach_requires_live_fd)
{
    fk_reset();
    ...
    ASSERT_EQ(hl_my_transport_attach_tls(t, sentinel), -1);
    t->fd = saved;
    ASSERT_EQ(hl_my_transport_close(t), 0);
    close(sv[1]);
    seam_clear();
 ...
 * + frees). */
static MyTransport *tp_make_live_op(void)
{
    g_fk_naddr = 1; g_fk_disp[0] = FK_PENDING;
    my_test_resolve = fk_resolve; my_test_socket_provider = &FK_PROVIDER;
    MyTransport *t = transport_new(&FK_PROVIDER);
    if (!t) return NULL;
 ...
 *    the exact contract hl_my_transport_close relies on. ────────────────────────── */
UTEST(mysql_transport_close, teardown_returns_status)
{
    fk_reset(); tp_arm_watchdog();
    MyTransport *t = tp_make_live_op();
    ASSERT_TRUE(t != NULL);

    my_test_force_no_detach = 1;
...
/* ── forced non-detachment: hl_my_transport_close PRESERVES the whole allocation
...
UTEST(mysql_transport_close, forced_non_detach_close_preserves_then_retry)
{
    fk_reset(); tp_arm_watchdog();
    MyTransport *t = tp_make_live_op();
    ASSERT_TRUE(t != NULL);
