...
UTEST(respwire_parse, simple_string) {
    PARSE("+OK\r\n");
    ASSERT_EQ((int)r, (int)HL_RESP_OK);
    ASSERT_EQ((int)v.type, (int)HL_RESP_STR);
...
UTEST(respwire_parse, error) {
    PARSE("-ERR bad\r\n");
    ASSERT_EQ((int)r, (int)HL_RESP_OK);
    ASSERT_EQ((int)v.type, (int)HL_RESP_ERR);
...
UTEST(respwire_parse, integer_neg) {
    PARSE(":-123\r\n");
    ASSERT_EQ((int)r, (int)HL_RESP_OK);
    ASSERT_EQ((int)v.type, (int)HL_RESP_INT);
...
UTEST(respwire_parse, bulk_null_resp2) {
    PARSE("$-1\r\n");
    ASSERT_EQ((int)r, (int)HL_RESP_OK);
    ASSERT_EQ((int)v.type, (int)HL_RESP_NULL);
}
...
UTEST(respwire_parse, boolean_and_double) {
    { PARSE("#t\r\n"); ASSERT_EQ((int)r, (int)HL_RESP_OK); ASSERT_EQ((int)v.type, (int)HL_RESP_BOOL); ASSERT_EQ(v.b, 1); }
    { PARSE(",3.5\r\n"); ASSERT_EQ((int)r, (int)HL_RESP_OK); ASSERT_EQ((int)v.type, (int)HL_RESP_DOUBLE); ASSERT_TRUE(v.d > 3.4 && v.d < 3.6); }
}
...
UTEST(respwire_parse, bignum_kept_as_text) {
    ...
    ASSERT_EQ((int)r, (int)HL_RESP_OK);
    ASSERT_EQ((int)v.type, (int)HL_RESP_BIGNUM);
}
...
UTEST(respwire_parse, array_of_bulk) {
    PARSE("*2\r\n$3\r\nfoo\r\n$3\r\nbar\r\n");
    ASSERT_EQ((int)r, (int)HL_RESP_OK);
    ASSERT_EQ((int)v.type, (int)HL_RESP_ARRAY);
...
UTEST(respwire_parse, nested_array_scan_reply) {
    ...
    ASSERT_EQ((int)v.type, (int)HL_RESP_ARRAY);
    ASSERT_EQ(v.arr.count, (size_t)2);
    ASSERT_EQ((int)v.arr.items[1].type, (int)HL_RESP_ARRAY);
...
UTEST(respwire_parse, map_resp3) {
    PARSE("%1\r\n$1\r\nk\r\n:7\r\n");
    ASSERT_EQ((int)r, (int)HL_RESP_OK);
    ASSERT_EQ((int)v.type, (int)HL_RESP_MAP);
    ASSERT_EQ(v.arr.count, (size_t)2);          /* 2*pairs */
    ASSERT_EQ((int)v.arr.items[1].type, (int)HL_RESP_INT);
...
UTEST(respwire_parse, need_more_incomplete_header) {
    ...
    ASSERT_EQ((int)r, (int)HL_RESP_NEED_MORE);
...
UTEST(respwire_parse, err_bad_bulk_length) {
    ...
    ASSERT_EQ((int)r, (int)HL_RESP_PARSE_ERR);
}
...
UTEST(respwire_parse, aggregate_without_arena_rejected) {
    ...
    ASSERT_EQ((int)r, (int)HL_RESP_PARSE_ERR);            /* no alloc -> reject aggregate */
}
