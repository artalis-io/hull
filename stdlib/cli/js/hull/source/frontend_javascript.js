...
const units = new Map();     // unit_id -> { ast, comments, linemap }
...
// declaration nodes at ANY nesting depth. Skips the annotation record keys (no `type`).
function collectDeclarations(ast, unit_id, linemap) {
    const out = [];
    function visit(node) {
...
// ---- analyze ----
export function analyze(bytes, path, opts) {
    try {
        const u = parse(bytes, { path: path || null });          // parser is never-raise
        const unit_id = ++nextUnitId;
        units.set(unit_id, { ast: u.ast, comments: u.comments, linemap: u.linemap });
        ...
        const declarations = collectDeclarations(u.ast, unit_id, linemap);
...
export function declarationSemantics(declId) {
    try {
        ...
        if (!units.has(r.unit_id)) return semErr("declaration's unit is no longer retained");
        if (r.formKind === "value") {
            const node = r.node;
            if (!node || node.type !== "VariableDeclaration") return semErr("node/kind mismatch: expected VariableDeclaration");
            ...
            const node = r.node;
            if (!node || node.type !== "FunctionDeclaration") return semErr("node/kind mismatch: expected FunctionDeclaration");
            if (!Array.isArray(node.params) || !node.body) return semErr("malformed function declaration (params/body)");
            ...
            const node = r.node;
            if (!node || node.type !== "ClassDeclaration") return semErr("node/kind mismatch: expected ClassDeclaration");
