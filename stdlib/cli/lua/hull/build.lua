...
int main(int argc, char **argv) { return hl_app_run(argc, argv); }
    ...
    -- Every produced app needs a STRONG hl_stdlib_feature_entries. The base
    -- default is weak and, resolved from libhull_platform.a (an archive), wins
    -- first, leaving the app with an empty runtime stdlib (require "hull.json"
    -- fails). Generate it as a direct object returning this app's runtime stdlib
    -- array (the base carries hl_stdlib_<rt>_entries; a lua app references only
    -- the lua array, a js app only the js array -- so the other runtime's stdlib
    -- is not pulled). Self-contained: HlEntry is only address-taken here, never
    -- dereferenced, so a layout-compatible local struct suffices (same
    -- self-contained convention as the by_hook feature_registry codegen).
    do
        local app_rt = file_exists(opts.app_dir .. "/app.lua") and "lua"
                    or (file_exists(opts.app_dir .. "/app.js") and "js" or nil)
        if app_rt then
            local sym = "hl_stdlib_" .. app_rt .. "_entries"
            local reg = table.concat({
                "/* Auto-generated by hull build - do not edit. */",
                "typedef __SIZE_TYPE__ size_t;",
                "typedef struct { const char *n; const unsigned char *d; unsigned int l; } HlEntry;",
                "extern const HlEntry " .. sym .. "[];",
                "static const HlEntry *const HL_STDLIB_FEATS[] = { " .. sym .. " };",
            ...
            write_file(tmpdir .. "/app_feature_stdlib.c", reg)
            if not tool.compiler.compile(tmpdir .. "/app_feature_stdlib.c",
                                         tmpdir .. "/app_feature_stdlib.o", nil) then
                tool.stderr("hull build: compilation failed (app_feature_stdlib.c)\n")
                tool.rmdir(tmpdir); tool.exit(1)
            end
            feature_objs[#feature_objs + 1] = tmpdir .. "/app_feature_stdlib.o"
        end
    end
