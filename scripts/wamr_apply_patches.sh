...
#
# Applies patches/wamr/0001 + 0002 onto a CLEAN checkout of the pinned base into
...
# --- tamper check: the applied tree must revert cleanly to the base -----------
git -C "$STAGED" apply --reverse --check --whitespace=nowarn "$ROOT/$P2" \
    || fail "applied tree does not reverse-match 0002 (tampered/extra content)"
...
echo "OK: pinned base $BASE + 0001 + 0002 -> $STAGED"
