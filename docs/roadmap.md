...
| **Demote `hull/log` and `hull/json` to declared modules** | **v0.1.1** | Honesty fix. The intrinsic-core exception should be the minimum needed to bootstrap an app — that's `hull/app` alone. `hull/log` (does I/O to stderr) and `hull/json` (pure but Hull-specific) belong with the rest of the declared stdlib. Migration: declare `"hull/log@1"` and/or `"hull/json@1"` in your manifest. The `res:json(...)` response helper and the runtime's internal logging stay intrinsic at the C level, so apps that don't call `log.*` or `json.*` directly need no change. |
