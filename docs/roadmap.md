...
| 3d  | Drop Keel entirely from HL_ENABLE_HTTP=0 | Planned | 3d | Replace Keel async (kl_async_op/suspend/complete, kl_timer_*, KlThreadPool) with a small poll-based primitives layer for CLI builds. Needed to reach the ~1.5MB binary-size target and to support async-in-app.main without a HTTP-server build. |
