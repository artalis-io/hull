...
| **Drop the unused Lua/JS runtime** | **feature** (runtime-less base + `lua`/`js`, auto-composed) | Slim a single-runtime app to one interpreter. Delivered as composable runtime *features* on a runtime-agnostic base (the GPU-backend pattern), NOT pre-built per-flavor libs: features compose M+N/M×N, so a runtime archive composes with any HTTP flavor, with no `full-lua × server-only × …` cross-product. Symmetric (drops either direction); exactly-one runtime, so it's auto-composed from the entry extension and embedded in `hull` (never `hull feature install`). ~0.5-1 MB. WASM slim is the same model, gated on a module + UDF two-signal check. Full spec: [roadmap_next.md](roadmap_next.md) "Runtimes as composable features." |
