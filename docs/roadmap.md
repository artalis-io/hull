...
| **flavor** | preset that turns default subsystems **off** to slim the base (subtractive) | `libhull_platform-<flavor>.a`, `--flavor=` | pure-compute |
...
| **Drop the unused Lua/JS runtime** | **feature** (runtime-less base + `lua`/`js`, auto-composed) | ✅ **SHIPPED (#113, 2026-07-26).** Slim a single-runtime app to one interpreter. Delivered as composable runtime *features* on a runtime-agnostic base (the GPU-backend pattern): a runtime archive composes with the **full** flavor, is auto-composed from the entry extension, and is embedded in `hull` (never `hull feature install`). ~0.5-1 MB. **Caveat:** the M+N-composes-M×N orthogonality does NOT yet hold for **reduced** flavors — the full-config runtime archive references HTTP caps + Keel that pure-compute drops, so `--flavor=<reduced>` × runtime (and `--with=tui` × runtime) fail closed pending **issue #114 (HTTP as a composable feature)**. WASM slim is the same model, gated on a module + UDF two-signal check. Full spec: [roadmap_next.md](roadmap_next.md) "Runtimes as composable features." |
