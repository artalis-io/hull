...
| ✅ **I** | **H1 + H2 + M5** — Agent surface cleanup, fully done in 3 commits. Step 1: `enumerate_routes` + `enumerate_middleware` vtable methods, `agent_routes_lua/js` collapsed to single-path. Step 2: `test_setup` + `run_test_file` vtable methods, shared `src/hull/test_runner.c` with pluggable writer (stdout for `hull test`, JSON for `hull agent test`); `agent_test_lua_ctx`/`_js_ctx` and `run_lua_tests`/`run_js_tests` all collapsed. Step 3: `agent_lib.c` (1113 lines) split into `agent/{routes,db,request,test,context,deploy}.c` + private `agent/{internal.h,helpers.c}` for shared helpers. | M | Three Lua/JS sibling pairs eliminated, test runner unified under one writer interface, every agent op gets its own focused file. `HlRuntimeVtable` now has the full set of introspection methods needed for a third runtime to slot in. |
| ✅ **J** | **M1** — Mark `HlLua`/`HlJS` internals private. Step 1: moved `HlLuaTimer`, `HlJSTimer`, `HlLuaWorkerDispatchOp`, `HlJsWorkerDispatchOp` + their helper functions (`hl_*_worker_dispatch_submit/op_free/op_free_all/cancel`, `hl_*_worker_db_init`) from `include/hull/runtime/{lua,js}.h` to `src/hull/runtime/{lua,js}/internal.h`. Step 2: added a Tier-4 stability block to the `HlLua`/`HlJS` struct comments in the public headers, explicitly marking fields below `base` as internal-and-unstable. Also refactored `agent/deploy.c` to route manifest extraction through the runtime vtable instead of peeking at `lua->L` / `js->ctx`. | M | Internal-only op structs no longer leak into the public surface. The struct bodies stay in the public headers (heap-factory migration deferred) so `app_context.c` and `tool.c` can keep stack-allocating; this is itself Tier-4 and can move post-v0.1.0 when a third-party consumer needs the opaque ABI. |
...
| ✅ **L** | **M9 + M10** — Share worker_db / get_buffer code between runtimes. **Scope landed:** extracted `hl_worker_db_get_and_prepare()` in `src/hull/worker_db.c` to fold the recurring "get worker DB + namespace check + `sqlite3_prepare_v2`" dance that Lua + JS both duplicated at the top of `worker_db_query` / `worker_db_exec`. Both runtimes now share that helper. **Scope intentionally NOT landed:** the per-cell column extraction loop (Lua `lua_pushinteger`/`lua_pushlstring` vs JS `JS_NewInt64`/`JS_NewStringLen`), the param-binding helpers, and `lua_get_buffer` vs `js_get_buffer` (M10) — all are 30–60 lines each of intrinsically runtime-API-shaped marshalling code. A unifying abstraction (HlValue-based row callback layer, virtual buffer-type dispatch) would cost more code than the duplication it removes, per the roadmap's own "reconsider if it becomes painful" guidance. It isn't. | M | One shared helper, ~12 lines net code reduction, single source of truth for the prepare/namespace pattern. The bigger refactor stays deferred. |
