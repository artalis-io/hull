...
- **Memory64 (engine-ready; cap-layer dispatch currently dark — see [#318](https://github.com/artalis-io/hull/issues/318)):** the design is `(memory i64 N)` modules detected automatically, with the runtime dispatching the 8-cell `(i64,i64,i64,i64)` calling convention; Memory64 is AOT-only (the fast interpreter doesn't support it) and `hull build` passes `--enable-memory64` to `wamrc` when it detects a Memory64 module. **However**, `WASM_ENABLE_MEMORY64` is defined only for the vendored WAMR objects, not for Hull's own translation units, so in the shipped build `cap/wasm.c`'s detection + 8-cell marshalling are `#if`-compiled out and `mod->is_memory64` is always 0. The WAMR engine and `wamrc` support Memory64; Hull's cap-layer plumbing to reach it is not yet compiled in. Tracked in #318.
...
| Memory64 | ⚠️ Engine-ready, cap-layer dispatch dark. `WASM_ENABLE_MEMORY64` is not defined for Hull TUs, so `cap/wasm.c` detection + 8-cell marshalling are compiled out in the shipped build ([#318](https://github.com/artalis-io/hull/issues/318)). The WAMR engine and `wamrc` support it |
