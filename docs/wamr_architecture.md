...
| Memory64 cap dispatch | ✅ Shipped. Detection via the public accessor `wasm_runtime_memory_is_memory64` (patch 0005); `memory64_requires_aot` guard + 8-cell AOT dispatch/readback CI-gated ([#318](https://github.com/artalis-io/hull/issues/318)). NOT yet: mapped spans (SPAN_INFO) on a 64-bit guest, and the standard `hull build` AOT path for a Memory64 plugin — [#334](https://github.com/artalis-io/hull/issues/334) |
