...
- **Shared data segments shipped:** `compute.segment(module, name, bytes)` loads up to 16 named read-only segments per module via WAMR's shared-heap mechanism. Multi-GB datasets readable zero-copy, with no per-access host call, by every instance (pooled and persistent). Throughput is workload- and codegen-dependent, not universally near-native; see the [mapped-span benchmark](mapped_span_benchmark_design.md) (segments and per-invocation spans share this read path).
...
| Memory64 (dispatch + mapped spans + `hull build`) | ✅ Shipped. Detection via the public accessor `wasm_runtime_memory_is_memory64` (patch 0005); `memory64_requires_aot` guard + 8-cell AOT dispatch/readback CI-gated ([#318](https://github.com/artalis-io/hull/issues/318)); SPAN_INFO mapped spans read a window >`UINT32_MAX` through the record's 64-bit `base`, CI-gated `memory64_span_readback` ([#334](https://github.com/artalis-io/hull/issues/334)); `hull build` of a Memory64 plugin works under the transparent shared-heap policy, CI-gated `e2e_compute_memory64.sh` on x86_64 + aarch64 ([#336](https://github.com/artalis-io/hull/issues/336)) |
