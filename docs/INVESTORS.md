...
The whole stack is vendored: Keel (HTTP server), Lua 5.4 + QuickJS (dual scripting), SQLite (database), mbedTLS (TLS client), TweetNaCl (Ed25519 signatures), pledge/unveil (kernel sandbox), miniz (gzip), and the Mozilla CA bundle (HTTPS without a system store). `hull build` is compiler-free by default (it emits the app object directly and links, so no separately-installed C compiler is required; the system linker is still used for the link step). Optional: WAMR for WASM compute, wgpu-native for GPU. No external package managers, no transitive dependency graphs, no runtime installation. The user double-clicks a file and has a working application. Their data never leaves their machine.
...
**Where Hull is today (as of 2026-05):** Hull v0.1.6 shipped on 2026-05-29 with signed binaries for Linux x86_64/aarch64, macOS arm64, and the universal Cosmopolitan APE, all downloadable from GitHub Releases. Shipped: dual runtime (Lua 5.4 + QuickJS), capability layer with manifest-declared fs/env/host/process gates, kernel sandbox (pledge/unveil on Linux/Cosmo/OpenBSD, Seatbelt on macOS), `hull build` with embedded TinyCC, WASM compute (interpreter + AOT + Memory64 + SIMD128), GPU compute (wgpu-native, Vulkan/Metal/DX12), the full stdlib (sessions, CSRF, RBAC, idempotency, outbox/inbox, FTS5, i18n, ETags, health, SMTP, WebSockets, SSE, background timers, audit logging), agent platform (~28 `hull agent` subcommands + MCP server), `hull update` with SHA-256 and release-signature enforcement, `hull tools install wamrc` for side-loading the AOT compiler, browser-side verifier at verify.gethull.dev, the gethull.dev landing site, and three completed security audits with all findings closed. **Trust chain end-to-end verifiable three independent ways as of v0.1.6:** (a) the existing three-layer Ed25519 chain (release + per-platform + per-app, with the embedded gethull release key live as of v0.1.3), (b) Sigstore + Rekor transparency log entry per release (verifiable via `cosign verify-blob`, independent of any gethull-managed key), (c) per-binary SLSA build-provenance attestations (verifiable via `gh attestation verify`, signed via Fulcio short-lived certs). Byte-reproducible builds (CI-gated on both Linux and macOS), live SBOM (`hull sbom`, four formats including CycloneDX and SPDX, published as signed release artifacts), and `hull verify-self` one-command binary verification. AGPL fork-and-rebrand path documented in `docs/fork_playbook.md`. The product exists, it ships, it's downloadable today, and procurement / compliance / sovereign-deployment buyers can verify every claim without trusting any single party. What remains for v1.0 is the multi-user server hardening, PostgreSQL backend, local-agent workbench, and ecosystem growth.
