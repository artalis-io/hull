...
| [`windows_e2e_status.md`](windows_e2e_status.md) | Windows e2e suite status: the last full 93-suite sweep (verdict per suite, with the failing ones and their causes), which suites are green only because they skip, the four recurring reasons a suite breaks on Windows, and how to run a sweep. |
...
| [`ssh_module_design.md`](ssh_module_design.md) | Phase 0 design report for a capability-secured `hull/ssh` client: dependency classification, the missing outbound-stream capability, and the stdlib additions it needs. |
| [`net_module_design.md`](net_module_design.md) | Design for `hull/net`, the capability-gated outbound byte-stream facility: API, capability shape, and the migration path for the SMTP/WS/HTTP clients that duplicate it privately today. |
...
| [`db_transport_extraction.md`](db_transport_extraction.md) | Decisions record for extracting the byte-identical `pg_transport.c` / `mysql_transport.c` (Slices 3/4) into one shared `cap/db_transport.{c,h}` (`HlDbTransport`): a behavior-neutral full merge (no shims), a diagnostic-only `"pg"`/`"mysql"` label, and composition via BOTH pull-by-symbol feature archives (no new archive) so the shared object is extracted exactly once and absent in a neither-build. |
| [`valkey_keel_transport_slice5.md`](valkey_keel_transport_slice5.md) | Decisions record for routing the Valkey/Redis client (`cap/valkey_conn.c`, the last raw-socket first-party client) onto the shared `HlDbTransport` (Slice 5): optional embedder `HlAllocator` on connect/adopt via `hl_alloc_kl`, a `set_io_timeout` preserving Valkey's `SO_*TIMEO` on the connect path only, implicit rediss TLS, the whole connection layer moved under `HL_VALKEY_NO_TLS` (dead "TLS not available" branch retired), and `cap_db_transport.o` as the valkey archive's third pull-by-symbol carrier. Includes the shared-TLS non-blocking-handshake fix (fail-closed fd-mode + monotonic deadline) surfaced by the rediss fail-closed test. |
| [`dns_resolver_keel_design.md`](dns_resolver_keel_design.md) | Design-only record scoping the one remaining raw network primitive after the socket migration: blocking `getaddrinfo` name resolution in `cap/db_transport.c` + `cap/smtp_transport.c`. Covers the `KlConnectOp` resolve seam and the **deadline-contract decision** it forces (DNS is deliberately outside the frozen D3 "TCP-establishment-only" and SMTP post-resolution `Dop` budgets, so async-in-op resolution needs a *separate* DNS deadline, not folding); Keel's `KlResolver` vtable + the capable hosted `kl_dns_resolver_create` (concurrent A/AAAA, multi-address, RFC 7766 TCP fallback; UDP-only only in the freestanding build); the central **NSS/nsswitch parity** tension (mDNS / LDAP / split-DNS), the resolver's borrowed-`KlEventCtx` ownership + per-transport cache futility, and the finding that Option A adds **no new sandbox authority** (Hull already grants `dns netlink unix`). Resting recommendation: keep the NSS-faithful blocking `getaddrinfo` (Option C); preferred triggered follow-up is an off-thread `getaddrinfo` `KlResolver` (Option B), with its ownership / capacity / shutdown contracts as first-class design work, not the built-in UDP resolver. |
