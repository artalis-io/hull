...
| **Ed25519 release signature (`hull.sha256.sig`)** | The manifest (hence every hash in it) was signed by Hull's offline release key, whose public half is source-controlled (`HL_RELEASE_PUBKEY_HEX` in `include/hull/release.h`) and embedded in every Hull binary, so an already-trusted Hull verifies a release without fetching a key from the current release channel. That continuity is the channel-independent root of trust. | It is only as good as the verifier: verifying it requires a hull binary you already trust or the pubkey obtained from a separately trusted channel. A freshly-downloaded hull verifying its own release is circular and is not an independent proof. |
