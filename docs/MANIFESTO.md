...
- **Defense in depth**. Six independent layers (seven with optional GPU compute), each enforced separately:
  ...
  5. **Digital signatures, three independent trust roots.** (a) Three Ed25519 layers covering the binary you downloaded: platform (gethull.dev key), app (developer key), and release (`hull.sha256.sig` verified by `hull update` against the embedded `HL_RELEASE_PUBKEY_HEX`). (b) Sigstore + Rekor transparency log entry for every release (`cosign verify-blob hull.sha256 --certificate hull.sha256.cosign.pem --signature hull.sha256.cosign.sig`), tied to the GitHub Actions OIDC identity of the release workflow. (c) SLSA build-provenance attestation per binary (`gh attestation verify hull-linux-x86_64 --repo artalis-io/hull`), signed via Fulcio short-lived cert. Any one is sufficient. A compromise of any single trust root leaves the other two intact. End-users self-verify their installed binary in one command: `hull verify-self`. The published SBOM (`hull.sbom.json` / `.cdx.json` / `.spdx.json`) is covered by all three signatures.
