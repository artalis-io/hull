...
| `e2e-cache-concurrent` | 3 of 11, **KNOWN** | Its premise is not met on this host. Of the 8 workers it races, **2 start**; the rest log NOTHING and never serve. In the mixed section **1 of 8** starts and the other 7 die on `failed to open database connection` - with no `-d` that DSN is `:memory:` (`app_context.c`), private per process, so it cannot be contention. Raising the boot wait from 2s to 20s moved it from 3-of-8 to 2-of-8: they are not slow, they do not start. None of this was visible because BOTH sections defined success as "none of `Segmentation|Abort|panic|fatal` appeared", so 7 dead workers scored zero matches and every cache invariant (one file per key, no zero-sized blobs, no leftover tmp) passed on a single writer's output while reporting 8 - the properties being checked are exactly the ones that only mean something under real concurrency. The suite now asserts the workers reached `listening on` and dumps the stderr of any that did not. **Why concurrent hull startup fails on Windows is unanswered** and is a Hull question, not a cache one; this suite is now honest about it rather than green for the wrong reason. |
