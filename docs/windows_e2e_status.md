...
| `e2e-ssh-tunnel` | 4 of 10, KNOWN | **A Hull bug, and the first coverage this code path has ever had on any platform.** `cap/net_stream.c` is the async outbound byte stream; its only consumer is `hull/ssh`, which had no e2e until this suite, so nothing exercised it anywhere. On a cosmo APE on Windows an outbound connect never completes. Traced: `socket() -> 12`, `connect(12, 127.0.0.1:P) -> EINPROGRESS`, then `ppoll({{3,POLLIN},{5,POLLIN},{12,POLLOUT,[POLLOUT]}}, 3, ...) -> 1` - the socket IS writable, so the connect finished - and the very next poll is `ppoll({{3,POLLIN},{5,POLLIN}}, 2, ...)` forever: fd 12 is deregistered and the parked coroutine is never resumed. The `timeout_ms` deadline does not fire either, so it hangs rather than erroring. NOT tunnel-specific and NOT new: a DIRECT `ssh.connect` with no relay hangs identically, so `hull/ssh` cannot connect at all on Windows. The 4 that pass are the two manifest-grant cases, which refuse BEFORE a socket is opened and so never reach the defect - a clean split that says the policy layer is fine and the transport resume is not. The suite bounds each app run at 20s so it FAILS rather than spending the job's budget. Promote it the moment the resume lands. |
...
| KNOWN | expected to FAIL (`e2e-project-discovery`, `e2e-ssh-tunnel`). A KNOWN suite that starts PASSING also fails the job - that is how it gets promoted rather than quietly drifting. |
