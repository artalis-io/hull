...
| `e2e-ssh-tunnel` | **now PASSES 10/10 (seam), ENFORCED** | The first reading here blamed Windows, and that was wrong. The suite hung, the trace showed `connect(12,..) -> EINPROGRESS` then `ppoll(..{12,POLLOUT,[POLLOUT]}..) -> 1` - the socket writable, the connect finished - and then fd 12 deregistered and nothing further, so it read as a cosmo/poll defect. It was neither: **two Hull bugs, on every platform**, and Linux CI hung identically once a suite finally drove the path. (1) `ssh_park` set `on_resume` but never called the backend's `op_suspend`, so `op_complete` found no per-op state and scheduled nothing. (2) Both async backends freed that state AFTER running the callback and then NULLed `_backend_state` - clobbering the state a re-suspend had just installed, because `on_resume` resumes a coroutine that runs to its next park and re-suspends the same op. No existing consumer re-enters (`http.fetch`, `compute.async`, `gpu.async` park once per operation); a byte stream parks on every read and write, so `hull/ssh` was the first to reach it. Fixed in `mod_ssh.c` and in both `async/keel.c` and `async/poll.c`. The live-sshd half still SKIPS here - Windows OpenSSH rejects the host key's NT ACLs and has no `UsePAM` - so Windows gates the tunnel seam and Linux/macOS gate the SSH session. |
...
| KNOWN | expected to FAIL (`e2e-project-discovery`). A KNOWN suite that starts PASSING also fails the job - that is how it gets promoted rather than quietly drifting. |
