...
| `e2e-project-discovery` | 3 of 66 | Two separate things. (1) The suite injects `$$` as a sidecar `session_pid` and expects hull's `kill(pid,0)` gate to call it live. On Windows hull is a NATIVE process and the shell an MSYS one, and they number processes differently - measured in one shell: `$$`=640, which Windows does not know, while its WINPID 36772 it does. So hull was right to call that pid dead. (2) **Do not simply pass the WINPID instead.** Tried, and it makes the suite stop 17 assertions early - at the first `agent inspect` against a sidecar carrying a LIVE pid - printing no summary and exiting 0, so `make` reports success. That is worse than the honest failure it replaces. Ruled out as causes: `set -e` plus the EXIT trap (measured, a succeeding trap does not swallow the status), hull signalling the pid (`agent.c` uses `kill(pid,0)`, existence only), and log truncation (the file ends cleanly). Unexplained, and it means hull's live-session fast path has likely never been exercised anywhere. Worth solving before touching these three again. |
